...
@LambdaHandler(lambdaName = "api_handler", roleName = "api_handler-role", aliasName = "learn", runtime = DeploymentRuntime.JAVA17, logsExpiration = RetentionSetting.SYNDICATE_ALIASES_SPECIFIED)
@EnvironmentVariables(value = {@EnvironmentVariable(key = "region", value = "${region}"), @EnvironmentVariable(key = "tables_table", value = "${tables_table}"), @EnvironmentVariable(key = "reservations_table", value = "${reservations_table}"), @EnvironmentVariable(key = "booking_userpool", value = "${booking_userpool}")})
public class APIHandler implements RequestHandler<APIHandler.APIRequest, APIGatewayV2HTTPResponse> {
    ...
    private Table buildTableObject(APIRequest apiRequest) {
        ...
        return new Table(Integer.valueOf(apiRequest.body_json().get("id")), Integer.valueOf(apiRequest.body_json().get("number")), Integer.valueOf(apiRequest.body_json().get("places")), Boolean.valueOf(apiRequest.body_json().get("isVip")), Objects.nonNull(apiRequest.body_json().get("minOrder")) ? Integer.parseInt(apiRequest.body_json().get("minOrder")) : null);
    }

    private Reservation buildReservationObject(APIRequest apiRequest) {
        System.out.println("Calling buildReservationObject ...");
        return new Reservation(Integer.valueOf(apiRequest.body_json().get("tableNumber")), apiRequest.body_json().get("clientName"), apiRequest.body_json().get("phoneNumber"), apiRequest.body_json().get("date"), apiRequest.body_json().get("slotTimeStart"), apiRequest.body_json().get("slotTimeEnd"));
    }

    private Table buildTableResponse(Map<String, AttributeValue> result) {
        return new Table(Integer.valueOf(result.get("id").getN()), Integer.valueOf(result.get("number").getN()), Integer.valueOf(result.get("places").getN()), result.get("isVip").getBOOL(), Objects.nonNull(result.get("minOrder")) ? (Integer.valueOf(result.get("minOrder").getN())) : null);
    }

    private Reservation buildReservationResponse(Map<String, AttributeValue> result) {
        return new Reservation(Integer.valueOf(result.get("tableNumber").getN()), result.get("clientName").getS(), result.get("phoneNumber").getS(), result.get("date").getS(), result.get("slotTimeStart").getS(), result.get("slotTimeEnd").getS());
    }

    private String createAppClient(String userPoolId) {
        System.out.println("Calling createAppClient ...");
        var result = identityProviderClient.createUserPoolClient(CreateUserPoolClientRequest.builder().userPoolId(userPoolId).explicitAuthFlows(ExplicitAuthFlowsType.ALLOW_ADMIN_USER_PASSWORD_AUTH, ExplicitAuthFlowsType.ALLOW_REFRESH_TOKEN_AUTH).clientName("api_client").build());
    ...
    private APIGatewayV2HTTPResponse signUpUser(APIRequest apiRequest, String userPoolId) {
        ...
        try {
            ...
            var adminCreateUserRequest = AdminCreateUserRequest.builder().temporaryPassword(apiRequest.body_json().get("password")).userPoolId(userPoolId).username(apiRequest.body_json().get("email")).messageAction(MessageActionType.SUPPRESS).userAttributes(userAttributeList).build();
    ...
    private APIGatewayV2HTTPResponse signInUser(APIRequest apiRequest, String userPoolId, String clientId) {
        ...
        var authRequest = AdminInitiateAuthRequest.builder().authFlow("ADMIN_USER_PASSWORD_AUTH").authParameters(Map.of("USERNAME", apiRequest.body_json().get("email"), "PASSWORD", apiRequest.body_json().get("password"))).userPoolId(userPoolId).clientId(clientId).build();
        ...
        try {
            ...
            if (Objects.nonNull(authResponse.challengeName()) && authResponse.challengeName().equals(ChallengeNameType.NEW_PASSWORD_REQUIRED)) {
                var adminRespondToAuthChallengeResponse = identityProviderClient.adminRespondToAuthChallenge(AdminRespondToAuthChallengeRequest.builder().userPoolId(userPoolId).clientId(clientId).session(authResponse.session()).challengeName(ChallengeNameType.NEW_PASSWORD_REQUIRED).challengeResponses(Map.of("NEW_PASSWORD", apiRequest.body_json().get("password"), "USERNAME", apiRequest.body_json().get("email"))).build());
    ...
    private String getUserPoolId() {
        ...
        try {
            ...
            userPoolDescriptionType = response.userPools().stream().filter(value -> value.name().equals(System.getenv("booking_userpool"))).findFirst().orElse(userPoolDescriptionType);
    ...
    private APIGatewayV2HTTPResponse scanTable() {
        try {
            var tableList = amazonDynamoDB.scan(new ScanRequest(System.getenv("tables_table"))).getItems().stream().map(this::buildTableResponse).toList();
    ...
    private APIGatewayV2HTTPResponse scanReservations() {
        try {
            var reservationList = amazonDynamoDB.scan(new ScanRequest(System.getenv("reservations_table"))).getItems().stream().map(this::buildReservationResponse).toList();
    ...
    private boolean validateTable(Reservation reservation) {
        var tableList = amazonDynamoDB.scan(new ScanRequest(System.getenv("tables_table"))).getItems().stream().map(this::buildTableResponse).filter(value -> reservation.tableNumber().equals(value.number())).count();
        System.out.println("Validate table:" + tableList);
        return tableList == 1;
    }

    private boolean validateReservation(Reservation reservation) {
        var reservationList = amazonDynamoDB.scan(new ScanRequest(System.getenv("reservations_table"))).getItems().stream().map(this::buildReservationResponse).filter(value -> value.tableNumber().equals(reservation.tableNumber()) && value.slotTimeStart().equals(reservation.slotTimeStart()) && value.slotTimeEnd().equals(reservation.slotTimeEnd())).count();
